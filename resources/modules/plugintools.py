...
ADDON           = xbmc.translatePath(os.path.join(PATH, 'addon.xml'))
#reload(sys)
#sys.setdefaultencoding('utf8')

module_log_enabled=True; http_debug_log_enabled=False; LIST="list"; THUMBNAIL="thumbnail"; MOVIES="movies"; TV_SHOWS="tvshows"; SEASONS="seasons"; EPISODES="episodes"; OTHER="other"; 
...
def add_item(action="",title="",plot="",url="",thumbnail="",fanart="",iconImage="",show="",episode="",extra="",page="",info_labels=None,isPlayable=False,folder=True):
    ...
    if url.startswith("plugin://"): itemurl=url; listitem.setProperty('IsPlayable','true')

    elif isPlayable:
        listitem.setProperty("Video","true"); listitem.setProperty('IsPlayable','true');
        try :
                itemurl='%s?action=%s&title=%s&url=%s&thumbnail=%s&plot=%s&extra=%s&page=%s'%(sys.argv[0],action,urllib.quote_plus(title),urllib.quote_plus(url),urllib.quote_plus(thumbnail),urllib.quote_plus(plot),
                    urllib.quote_plus(extra),urllib.quote_plus(page))

        except Exception as e :
                if 'KeyError' in str(e):                     
                        urllib.quote_plus(title.encode('utf-8'), safe=':/'.encode('utf-8'))
                        urllib.quote_plus(url.encode('utf-8'), safe=':/'.encode('utf-8'))
                        itemurl='%s?action=%s&title=%s&url=%s&thumbnail=%s&plot=%s&extra=%s&page=%s'%(sys.argv[0],action,title,url,urllib.quote_plus(thumbnail),plot,urllib.quote_plus(extra),urllib.quote_plus(page))
                else :
        ...
        try :
                ...
                if 'KeyError' in str(e):
                        ...
                        urllib.quote_plus(title.encode('utf-8'), safe=':/'.encode('utf-8'))
                        urllib.quote_plus(url.encode('utf-8'), safe=':/'.encode('utf-8'))
                        urllib.quote_plus(extra.encode('utf-8'), safe=':/'.encode('utf-8'))
                        itemurl='%s?action=%s&title=%s&url=%s&thumbnail=%s&plot=%s&extra=%s&page=%s'%(sys.argv[0],action,title,url,urllib.quote_plus(thumbnail),urllib.quote_plus(plot),extra,urllib.quote_plus(page))
...
def add_itemcontext(action="",title="",plot="",url="",thumbnail="",fanart="",iconImage="",show="",episode="",extra="",page="",info_labels=None,contextmenu=None,isPlayable=False,folder=True):
    ...
    if url.startswith("plugin://"): itemurl=url; listitem.setProperty('IsPlayable','true')
        ...
        try :
                ...
                if 'KeyError' in str(e):
                        ...
                        urllib.quote_plus(title.encode('utf-8'), safe=':/'.encode('utf-8'))
                        urllib.quote_plus(url.encode('utf-8'), safe=':/'.encode('utf-8'))
                        itemurl='%s?action=%s&title=%s&url=%s&thumbnail=%s&plot=%s&extra=%s&page=%s'%(sys.argv[0],action,title,url,urllib.quote_plus(thumbnail),urllib.quote_plus(plot),urllib.quote_plus(extra),urllib.quote_plus(page))
    ...
    if not contextmenu == None: listitem.addContextMenuItems(contextmenu)

    xbmcplugin.addDirectoryItem(handle=int(sys.argv[1]),url=itemurl,listitem=listitem,isFolder=folder)


def close_item_list(): _log("close_item_list"); xbmcplugin.endOfDirectory(handle=int(sys.argv[1]),succeeded=True,cacheToDisc=False)
...
def play_resolved_url(url,title=''):
    ...
    if title : listitem.setLabel(title)
...
def direct_play(url,title=""):
    ...
    xlistitem.setInfo("video",{"Title":title}); playlist=xbmc.PlayList(xbmc.PLAYLIST_VIDEO); playlist.clear(); playlist.add(url,xlistitem);xbmcPlayer=xbmc.Player(); xbmcPlayer.play(playlist)
...
# Kill Kodi
def killxbmc():
    choice = plugintools.message_yes_no(
    ...
    if myplatform == 'osx':
        ...
        try:
            ...
            pass
        plugintools.message(AddonTitle, "If you\'re seeing this message it means the force close",
        ...
        try:
            ...
            pass
        plugintools.message(AddonTitle, "If you\'re seeing this message it means the force close",
                            "was unsuccessful. Please force close XBMC/Kodi [COLOR=lime]DO NOT[/COLOR] exit cleanly via the menu.", "Use task manager and NOT ALT F4")
    else:
        ...
        try:
            ...
            pass
        plugintools.message(AddonTitle, "If you\'re seeing this message it means the force close",
                            "was unsuccessful. Please force close XBMC/Kodi [COLOR=lime]DO NOT[/COLOR] exit via the menu.", "iOS detected.  Press and hold both the Sleep/Wake and Home button for at least 10 seconds, until you see the Apple logo.")
...
f=open(ADDON); data=f.read(); f.close()
