...
vijayVODUrl_ori = f'''http://api.yamsonline.com/astream?name=hotstarnew&username={username}&password={password}'''
...
def __get_icon(title):
    ...
    icon_path = xbmcvfs.translatePath(
        os.path.join(Path, 'resources', 'images', f'''{title}.png'''))
...
def history(params):
    ...
    for item in sorted_date:
        ...
        if item['film_id'] == 0:
            if 'mp4' in label:
                vod = label.replace('\\', '')
                vurl = f'''http://api.yamsonline.com/playvod?username={username}&password={password}&name=media/{vod}'''
...
def latestMovies(params):
    ...
    for item in items:
        ...
        plugintools.add_itemcontext(action='show_movie_files', title=item['label'], url=item['id'],
                                    ...
                                    page='0', contextmenu=context_men, folder=True)
    xbmcplugin.endOfDirectory(int(sys.argv[1]), cacheToDisc=True)
...
def play_indian_channel(params):  # id, cat, iconimg, name):
    ...
    if not authenticated:
        ...
        if timezone is None:
            ...
            url = f'http://vod.yamsftp.net/playlist.php?cfg={cat}&chanid={id}&date={now.month}/{now.day}/{now.year}&time={str(now.hour).zfill(2)}{str(now.minute).zfill(2)}'
...
def catchupvod_lang(params):
    ...
    if xbmc.getCondVisibility('!Skin.HasSetting(HomeMenuNoBasicButton)'):
        plugintools.add_item(action='catchupvod_channels', url=yuppUrl + "tamil", title='Tamil',
                             thumbnail=__get_icon('tamil_movies_tv'))
    plugintools.add_item(action='catchupvod_channels', url=yuppUrl + "telugu", title='Telugu',
                         thumbnail=__get_icon('telugu_movies_tv'))
    plugintools.add_item(action='catchupvod_channels', url=yuppUrl + "malayalam", title='Malay',
...
def refreshpvr(params):
    ...
    xbmc.executeJSONRPC(
        '{"jsonrpc":"2.0","id":1,"method":"Addons.SetAddonEnabled","params":{"addonid":"pvr.demo","enabled":true}}')
    xbmc.executeJSONRPC(
        '{"jsonrpc":"2.0","id":1,"method":"Addons.SetAddonEnabled","params":{"addonid":"pvr.demo","enabled":false}}')
...
def correctpvr():
    ...
    if RAM < 999:
        choice = xbmcgui.Dialog().yesno('[COLOR white]Low Power Device [COLOR lime]RAM: ' + RAMM + '[/COLOR][/COLOR]',
                                        '[COLOR white]Your device has been detected as a low end device[/COLOR]',
                                        '[COLOR white]We recommend avoiding PVR usage for this reason[/COLOR]',
                                        '[COLOR white]We cannnot support low end devices for PVR[/COLOR]',
                                        nolabel='[COLOR lime]OK, Cancel this[/COLOR]',
                                        yeslabel='[COLOR red]I know, proceed[/COLOR]')
        if choice == 0:
            sys.exit(1)
        elif choice == 1:
            pass
    ...
    dialog.ok("[COLOR white] KODI Notification [/COLOR]",
              '[COLOR white]A number of repeating pop up messages will appear shortly after kodi will exit[/COLOR]',
              ' ', '[COLOR white]You [B]MUST[/B] click OK, and hereafter restart Kodi[/COLOR]')
...
def show_launch(params):
    if not xbmc.getCondVisibility('Pvr.HasTVChannels'):
        dialog.ok("[COLOR white] AstreamWeb PVR Services [/COLOR]", '[COLOR white]No Provider Found[/COLOR]', ' ',
                  '[COLOR white]Please select a provider on the following pop up window[/COLOR]')
        show_correctpvr()
    else:
        if not isiptvauth_ok():
            return
...
def latestMovieshome(params):
    ...
    listitems = []
    for item in items:
        path = 'plugin://plugin.video.yams/?action=list_home_movie&title={}&url={}&thumbnail={}&plot={}&extra={}&page={}'.format(
            item['title'], item['id'], '', '', '', '0')
