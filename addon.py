...
import base64,os,re,unicodedata,time,string,sys,urllib,urllib2,json,urlparse,datetime,zipfile,shutil,io,requests,subprocess,calendar,hashlib
import StringIO, logging, random, array, htmllib
...
fanart          = xbmc.translatePath(os.path.join(Path, ''))
...
def run():
    if xbmc.getCondVisibility("System.Platform.Android") == 1:
        if not os.path.exists(xbmc.translatePath('/sdcard/Android/data/com.androidtoid.com/')):
                os.mkdir(xbmc.translatePath('/sdcard/Android/data/com.androidtoid.com/'))
...
def home2(params):
	...
	# astreamiptv
	path = xbmc.translatePath(os.path.join('special://home/userdata/addon_data/plugin.video.astreamiptv', '')).encode('string-escape')
	...
	path = xbmc.translatePath(os.path.join('special://home/userdata/addon_data/plugin.video.astreamiptv2', '')).encode('string-escape')
...
def __get_icon(title):
	...
	icon_path = xbmc.translatePath(os.path.join(Path, 'resources', 'images','%s.png' %title))
	#print icon_path
...
def username1():
	if xbmc.getCondVisibility("System.Platform.Android") == 1:
		usernamePath = os.path.join(xbmc.translatePath(os.path.join('/sdcard/Android/data/com.androidtoid.com/', '')).encode('string-escape'), "username")
	elif (xbmc.getCondVisibility("System.Platform.Windows") == 1):
		usernamePath = os.path.join(xbmc.translatePath(os.path.join(os.getenv('APPDATA'),'')).encode('string-escape'), "username")
	elif (xbmc.getCondVisibility("system.platform.tvos") == 1):
		usernamePath = os.path.join(xbmc.translatePath(os.path.join('special://home/userdata/', '')).encode('string-escape'), "username")
		...
		if not os.path.exists(pathf):
			os.mkdir(pathf)
		usernamePath = os.path.join(xbmc.translatePath(os.path.join(os.getenv('HOME') + "/Library/Application Support/OSConfig/",'')).encode('string-escape'), "username")
...
def boxname(params):
	if xbmc.getCondVisibility("System.Platform.Android") == 1:
		boxnamePath = os.path.join(xbmc.translatePath(os.path.join('/sdcard/Android/data/com.androidtoid.com/', '')).encode('string-escape'), "boxname")
	elif (xbmc.getCondVisibility("System.Platform.Windows") == 1):
		boxnamePath = os.path.join(xbmc.translatePath(os.path.join(os.getenv('APPDATA'),'')).encode('string-escape'), "boxname")
	elif (xbmc.getCondVisibility("system.platform.tvos") == 1):
		boxnamePath = os.path.join(xbmc.translatePath(os.path.join('special://home/userdata/', '')).encode('string-escape'), "boxname")
	elif (xbmc.getCondVisibility("system.platform.osx") == 1):
		pathf = xbmc.translatePath(os.path.join(os.getenv('HOME') + "/Library/Application Support/OSConfig/",'')).encode('string-escape')
		if not os.path.exists(pathf):
			os.mkdir(pathf)
		boxnamePath = os.path.join(xbmc.translatePath(os.path.join(os.getenv('HOME') + "/Library/Application Support/OSConfig/",'')).encode('string-escape'), "boxname")
...
def password1():
	if xbmc.getCondVisibility("System.Platform.Android") == 1:
		passwordPath = os.path.join(xbmc.translatePath(os.path.join('/sdcard/Android/data/com.androidtoid.com/', '')).encode('string-escape'), "password")
	elif (xbmc.getCondVisibility("System.Platform.Windows") == 1):
		passwordPath = os.path.join(xbmc.translatePath(os.path.join(os.getenv('APPDATA'),'')).encode('string-escape'), "password")
	elif (xbmc.getCondVisibility("system.platform.tvos") == 1):
		passwordPath = os.path.join(xbmc.translatePath(os.path.join('special://home/userdata/', '')).encode('string-escape'), "password")
	elif (xbmc.getCondVisibility("system.platform.osx") == 1):
		pathf = xbmc.translatePath(os.path.join(os.getenv('HOME') + "/Library/Application Support/OSConfig/",'')).encode('string-escape')
		if not os.path.exists(pathf):
			os.mkdir(pathf)
		passwordPath = os.path.join(xbmc.translatePath(os.path.join(os.getenv('HOME') + "/Library/Application Support/OSConfig/",'')).encode('string-escape'), "password")
	...
	if authenticated:
		...
		if (currentUUID1 != username):
			try:
				...
				print('<<< traceback end <<<')
		# Check first run
		path = xbmc.translatePath(os.path.join('special://home/userdata', '')).encode('string-escape')
		configFile1 = os.path.join(path, 'astreamweb.config')
		configUrl1 = 'https://astreamweb.com/kodi/astreamweb.config'

		response = urllib2.urlopen("https://astreamweb.com/kodi/astreamweb.conf").read()
...
def astreamweb_update(params):
	updater.upgrade_astreamweb(xbmc.translatePath(os.path.join(dataPath, "tmp.apk")))
...
def history(params):
	...
	response = urllib2.urlopen(vurl).read()
...
def api_yamsonline_providers (params):
		...
		if not authenticated:
				...
				xbmc.log(sourceurl)
				response = urllib2.urlopen(sourceurl).read()
				json_data = json.loads(response)
...
def play_iptv_favourite(params):#url, label, cid):
	...
	if cid == '0':
		response = urllib2.urlopen(base64.b64decode(url))
		html = response.read()
		#print(html)
                ...
                if str(i["movie_id"]) == page  :
                        ...
                        for x in i["files"] :
                                ...
                                if (  ("mkv" == sname ) or ( "mp4" == sname )) or  (("m4v" == sname ) or  ("avi" == sname )) :
                                        if i["radarr"]  :
                                                url = 'https://api.yamsonline.com/playpersonal?id=%s&title=%s&username=%s&password=%s&name=%s'%(page,urllib.quote_plus(path11.encode('utf-8')),username,password,urllib.quote_plus(name.encode('utf-8')))
                                        else :
                                                url = 'https://api.yamsonline.com/playmovie?id=%s&username=%s&password=%s&name=%s'%(page,username,password,urllib.quote(name))
...
def play_vod(params):
	...
	time.sleep(3)
        if xbmc.Player().isPlaying() == False:
                response = urllib2.urlopen(urllink).read()
                json_data = json.loads(response)
...
def show_hotstar_ori(params):
	...
	if not isauth_ok():
		return
	response = urllib2.urlopen(vijayVODUrl_ori).read()
...
def show_hotstarplayvideo(params):
	...
	if not authenticated:
		...
		import urlparse
		parsedUrl = urlparse.parse_qs(urlparse.urlparse(vurl).query)
...
def show_indian_channels(params):
	...
	#channelsJson = network.Net().http_GET("http://vod.yamsftp.net/apiyamsnew.php?cfg={0}".format(category))
        channelsJson = urllib2.urlopen(link).read()
	...
	if not authenticated:
		...
		timezone = get_timezone_in_hours()
			...
			return plugintools.play_resolved_url("") #xbmcplugin.endOfDirectory(int(sys.argv[1]))
                        ...
                        playlist.clear()
                        playlist.add(url)
                        itemToPlay = xbmcgui.ListItem(iconImage=urllib.unquote(iconimg), label=name)
...
def secure_catchupvod_url(src_url):
	secret = "psanni56"
	#print(src_url)
	url = urlparse.urlparse(src_url).path
	#if url[0] == "/": url = url[1:]
...
def multiSearchLangs(params):
	...
	if path == "-":
		...
		path += "+"
	years = range(1950, dt.datetime.now().year+1)
	years.reverse()
	for year in years :
...
def multiSearchYears(params):
	...
	nodes.remove({u'name': u'Bluray', u'id': u'72'})
	for node in nodes :
...
def font_size(params):
	path = xbmc.translatePath(os.path.join('special://home/addons/skin.estuary/1080i', '')).encode('string-escape')
	dialog = xbmcgui.Dialog()
...
def select_skin_language(langs=None):
	...
	categories = [int(a) for a in jsonResp["categories"].keys()]
	category = ""
	#print(categories)
	...
	catPath = xbmc.translatePath(os.path.join('special://home/userdata', '')).encode('string-escape')
	...
	if not os.path.exists(xbmc.translatePath('special://home/userdata/addon_data/')):
		os.mkdir(xbmc.translatePath('special://home/userdata/addon_data/'))
	if not os.path.exists(xbmc.translatePath('special://home/userdata/addon_data/script.skinshortcuts/')):
		os.mkdir(xbmc.translatePath('special://home/userdata/addon_data/script.skinshortcuts/'))
	path = xbmc.translatePath(os.path.join('special://home/userdata/addon_data/script.skinshortcuts/', '')).encode('string-escape')
...
def __get_per_page():
	...
	if per_page not in (range(1, 200)):
		per_page = 200
...
def correctpvr():
	...
	categoriesContent = urllib2.urlopen(categoriesUrl).read()
	categoriesJson = json.loads(categoriesContent)
	categories = categoriesJson.keys()
	categories.sort()
	dialog = xbmcgui.Dialog()
